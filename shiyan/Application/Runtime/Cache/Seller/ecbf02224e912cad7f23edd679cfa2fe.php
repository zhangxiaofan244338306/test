        ...
        <div class="container-fluid" style="padding-right: 0px; padding-left: 0px; ">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">编辑订单促销活动</h3>
                </div>
                <div class="panel-body ">   
                    <!--表单数据-->
                    <form method="post" id="promotion" action="<?php echo U('Promotion/prom_order_save');?>">                    
                        <!--通用信息-->
                        ...
                        <div class="tab-pane active" id="tab_tongyong">                           
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    ...
                                    <td class="col-sm-8">
                                        <input type="text" class="form-control" name="money" id="money" value="<?php echo ($info["money"]); ?>"  onpaste="this.value=this.value.replace(/[^\d.]/g,'')" onkeyup="this.value=this.value.replace(/[^\d.]/g,'')"/>
                                        <span id="err_attr_name" style="color:#F00; display:none;"></span>                                        
                                    </td>
                                </tr>
                                <tr id="expression">
                                	<td class="col-sm-2">折扣：</td>
                                	<td><input type="text" name="expression"  value="<?php echo ($info["expression"]); ?>" class="" style="witdh:150px" onpaste="this.value=this.value.replace(/[^\d.]/g,'')" onkeyup="this.value=this.value.replace(/[^\d.]/g,'')" /><label>% 折扣值(1-100 如果打9折，请输入90)</label> </td>
                                </tr>
                                <tr>
                                    <td>开始时间：</td>
                                    <td>
                                ...
                                <tr>
                                    <td>适合用户范围：</td>
                                    <td>
                                    	<?php if(is_array($level)): foreach($level as $key=>$vo): ?><input type="checkbox" <?php if(strripos($info['group'],$vo['level_id']) !== false): ?>checked<?php endif; ?> name="group[]" value="<?php echo ($vo["level_id"]); ?>"><?php echo ($vo["level_name"]); ?>
...
<script>
    ...
    $(function () {
        ...
        // 这里暂时取消，手机上显示，还有图片上传有困难，以后再开放这里
    });
...
$("#prom_type").on("change",function(){
	  ...
	  switch(type){
	    ...
	    case 2:{
	      expression = '<td><b class="red">*</b>积分：</td> <td> <input name="expression" type="text" class="small"  pattern="int" value=""> <label>订单送积分额度!</label> </td>';
	      break;
	    }
	    case 3:{
	      expression = '<td><b class="red">*</b>代金券：</td> <td><select name="expression"><tpshop sql="select * from __PREFIX__coupon where type=0" item="v"><option value="<?php echo ($v["id"]); ?>" <?php if($v[id] == $info[expression]): ?>selected<?php endif; ?>><?php echo ($v["name"]); ?></option></tpshop></select></td>';
	      break;
	    }
	    case 4:{
	      expression = '';
