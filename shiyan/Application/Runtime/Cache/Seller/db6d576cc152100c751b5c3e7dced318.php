      ...
      <div class="panel panel-default">
        <div class="panel-body" style="padding: 0px">
          <div class="navbar navbar-default">
              <form action="<?php echo U('Promotion/search_goods',array('tpl'=>'select_goods'));?>" id="search-form2" class="navbar-form form-inline" method="get">
                <div class="form-group">
                  <select name="cat_id" id="cat_id" class="form-control">
                    <option value="">所有分类</option>
                        <?php if(is_array($categoryList)): foreach($categoryList as $k=>$v): if(in_array($v[id],$bind_class_id)): ?><option value="<?php echo ($v['id']); ?>" <?php if($v[id] == $cat_id): ?>selected<?php endif; ?>><?php echo ($v['name']); ?></option><?php endif; endforeach; endif; ?>
                  </select>
                </div>
                <div class="form-group">
			        ...
			        <table class="table table-bordered table-hover">
			            ...
			            <tbody id="goos_table">
			                <?php if(is_array($goodsList)): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                               	<td class="text-left">                
                                    <input type="radio" name="goods_id" data-id="<?php echo ($list["goods_id"]); ?>" data-name="<?php echo ($list["goods_name"]); ?>" data-count="<?php echo ($list["store_count"]); ?>" onclick="changeGoods(this)"/>
                                </td>
