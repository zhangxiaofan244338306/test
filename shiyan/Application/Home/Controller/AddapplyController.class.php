...
class AddapplyController extends BaseController {
	...
	public function contact(){
		if($this->apply['apply_state'] == 1) redirect(U('Addapply/apply_info'));
		if(IS_POST){
			$data = I('post.');
			if(empty($this->apply)){
				...
				if(M('store_apply')->add($data)){
					if($data['apply_type'] == 0){
						redirect(U('Addapply/basic_info'));
					}else{
						redirect(U('Addapply/basic_info',array('apply_type'=>1)));
					}
				}else{
				...
				redirect(U('Addapply/basic_info',array('apply_type'=>$data['apply_type'])));
			}
		}
	...
	public function basic_info(){
		...
		if(IS_POST){
			...
			M('store_apply')->where(array('user_id'=>$this->user_id))->save($data);
			redirect(U('Addapply/seller_info'));
		}
	...
	public function agreement(){
		if(!empty($this->apply)){
			if($this->apply['apply_state'] == 1){
				redirect(U('Addapply/apply_info'));
				...
				if($this->apply['apply_type'] == 1){
					redirect(U('Addapply/basic'));
				}else{
					redirect(U('Addapply/seller_info'));
				}
			}else if($this->apply['apply_state'] == 0 && empty($this->apply['business_licence_cert'])){
				redirect(U('Addapply/remark'));
			}else{
				redirect(U('Addapply/apply_info'));
			}
		}
		if(IS_POST){
			redirect(U('Addapply/contact'));
		}		
		$this->display();
	}
	...
	public function remark(){
		...
		if(IS_POST){
			...
			M('store_apply')->where(array('user_id'=>$this->user_id))->save($data);
			$this->success('提交成功',U('Addapply/apply_info'));
		}
