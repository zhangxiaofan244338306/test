...
class Plmshop extends TagLib {
	protected $tags = array(
		'adv' => array('attr'=>'limit,order,where,item','close'=>1),
		'article' => array('attr'=>'limit,order,where','close'=>1),
                'universal' => array('attr'=>'sql,key,item,result_name,t','close'=>1,'level'=>3), // universal sql 万仑标签
	...
	public function _adv($tag,$content){
        ...
        $str = '<?php ';
        $str .= '$pid ='.$pid.';';
        $str .= '$ad_position = M("ad_position")->cache(true,PLM_CACHE_TIME)->getField("position_id,position_name,ad_width,ad_height");';
        $str .= '$result = D("ad")->where("pid=$pid  and enabled = 1 and start_time < '.strtotime(date('Y-m-d H:00:00')).' and end_time > '.strtotime(date('Y-m-d H:00:00')).' ")->order("orderby desc")->cache(true,PLM_CACHE_TIME)->limit("'.$limit.'")->select();';
        $str .= '
...
foreach($result as $'.$key.'=>$'.$item.'):       
	...
	*/		
	 ...
	 * @return string
	 */
	public function _universal($tag,$content){
            $sql = $tag['sql']; // sql 语句     
           ...
           // $sql = str_replace(')', '."', $sql);
            ...
            $t  =  !empty($tag['t']) ? $tag['t'] : PLM_CACHE_TIME;// 缓存时间
